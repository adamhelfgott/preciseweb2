...
import { advertisersHowItWorksQuery } from "@/sanity/lib/queries";
...
export default function HowItWorksSectionWithCMS() {
  // Fetch how it works data from Sanity
  const { data: howItWorksData } = useSanityData<HowItWorksData>(advertisersHowItWorksQuery);

  // Fallback to hardcoded content
  const data = howItWorksData || {
    headline: "Query intelligence without accessing data",
    steps: [
      {
        number: 1,
