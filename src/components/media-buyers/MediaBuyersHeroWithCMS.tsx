...
import { mediaBuyersHeroQuery } from "@/sanity/lib/queries";
...
export default function MediaBuyersHeroWithCMS() {
  // Fetch hero data from Sanity
  const { data: heroData } = useSanityData<HeroData>(mediaBuyersHeroQuery);

  // Fallback to hardcoded content
  const hero = heroData || {
    headline: "Drive better outcomes with verified audiences",
    subheadline: "Access premium data segments with built-in performance validation. Optimize campaigns across the entire funnel with transparent attribution that proves what's actually working.",
    primaryButtonText: "Access audiences",
    primaryButtonHref: "/get-started",
    secondaryButtonText: "See all outcomes",
    secondaryButtonHref: "#outcomes",
  };
